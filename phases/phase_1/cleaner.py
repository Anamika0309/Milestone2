"""
Phase 1.3 - Cleaner & Normalizer Module (Standalone Phase Copy)
...
try:
    ...
    try:
        ...
        from mf_faq.ingestion.extractor import ExtractedDoc
    ...
    def clean_document(self, doc: ExtractedDoc) -> CleanedDoc:
        ...
        try:
            ...
            for section_name, section_text in doc.sections.items():
                cleaned_text = self.clean_section(section_text.get('text', ''), section_name)
                if cleaned_text:
                    cleaned_sections[section_name] = {
                        'name': section_name,
                        'text': cleaned_text,
                        'source': section_text.get('source', 'html_section')
                    }
            ...
            for scheme_dir_name in os.listdir(self.output_dir):
                ...
                if os.path.isdir(scheme_dir):
                    ...
                    if os.path.exists(cleaned_file):
                        ...
                        if not health_report['last_cleaning_time'] or file_mtime > health_report['last_cleaning_time']:
                            health_report['last_cleaning_time'] = datetime.fromtimestamp(file_mtime, timezone.utc).isoformat()
                    else:
