"""
Phase 1.5 - Embedder Module (Standalone Phase Copy)
...
# Safe library import verification with inline fallback
try:
    ...
    try:
        ...
        # Inline fallback implementation for standalone copy
        import sys
        import subprocess
...
class Embedder:
    ...
    def generate_embeddings(self, chunks: List[Dict[str, Any]]) -> List[List[float]]:
        ...
        if not self.model:
            logger.warning("Using mock embedding generator (SentenceTransformers not available or not safe)")
            ...
            for chunk in chunks:
                ...
                # Create a pseudo-random list of floats based on the hash
                emb = []
                for i in range(self.embedding_dim):
                    val = float(int(h[i % len(h)], 16)) / 15.0
                    emb.append(val)
    ...
    def save_model_metadata(self) -> bool:
        ...
        try:
            metadata = {
                ...
                'model_version': getattr(self.model, 'version', '1.0') if self.model else 'mock-1.0',
            ...
            # Save embeddings to ChromaDB if available
            try:
                try:
                ...
                if ChromaVectorStore is not None:
                    ...
                    # Note: adding embeddings requires chunk items, which is handled during embed_all
                    logger.info("ChromaVectorStore detected and available")
            except Exception as e:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            health_report = {
                'model_loaded': self.model is not None or not SENTENCE_TRANSFORMERS_AVAILABLE,
                'model_name': self.model_name,
                'embedding_dim': self.embedding_dim,
                'total_chunks': 0,
            ...
            # Determine health
            if health_report['embeddings_file_exists'] and health_report['metadata_file_exists']:
