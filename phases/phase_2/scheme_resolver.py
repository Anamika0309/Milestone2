"""
Phase 2.2 - Scheme Resolver (Standalone Phase Copy)
...
class SchemeResolver:
    ...
    def __init__(self, config_dir: str = None):
        ...
        logger.info(f"Initialized standalone scheme resolver with {len(self.scheme_names)} schemes (Config: {self.config_dir})")
    ...
    def detect_scheme_terms(self, query: str) -> Dict[str, Any]:
        ...
        for scheme_id, keywords in self.scheme_keywords.items():
            for keyword in keywords:
                if keyword in query_lower:
                    detected_schemes.add(scheme_id)
        ...
        return {
            'detected_schemes': list(detected_schemes),
