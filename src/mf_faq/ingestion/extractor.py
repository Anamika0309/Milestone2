...
class Extractor:
    ...
    def extract_with_trafilatura(self, html_content: str, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not extracted:
                return {'error': 'bare_extraction returned None'}
            
            result = {
                'title': getattr(extracted, 'title', '') or '',
                'author': getattr(extracted, 'author', '') or '',
                'main_content': getattr(extracted, 'text', '') or '',
                'sections': {}
            }
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if max_mtime > 0:
                health_report['last_extraction_time'] = datetime.fromtimestamp(max_mtime, timezone.utc).isoformat()
